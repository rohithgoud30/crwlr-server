...
def find_terms_link(soup: BeautifulSoup, base_url: str) -> str:
    """
    Scan anchors in the page for terms of service related links
    ...
    """
    # Terms-specific patterns - MUST include at least one of these in text or href
    required_terms_matches = [
        "terms of service", "terms of use", "terms and conditions",
        "user agreement", "service agreement", "legal agreement",
        "platform agreement", "website terms", "site terms",
        "legal terms", "legal information", "terms", "tos", "eula",
        "general terms", "conditions of use", "condition of use",
        "legal notice", "website agreement", "site usage", "website usage"
    ]

    # URL path fragments that strongly indicate a terms page
    terms_path_indicators = [
        "/terms", "/terms-of-service", "/terms-of-use", 
        "/terms-and-conditions", "/tos", "/legal/terms", 
        "/legal", "/terms-conditions", "/user-agreement", 
        "/eula", "/legal/terms-of-service", "/legal/terms-of-use", 
        "/legal/user-agreement", "/policies/terms", 
        "/policies/terms-of-service", "/policies/terms-of-use", 
        "/about/terms", "/about/legal", "/info/terms",
        "/legal-terms", "/legal-information", "/legal-notice",
        "/legal/tos", "/help/terms", "/help/tos", "/help/legal"
    ]

    # URL patterns that typically indicate help center or support pages containing terms info
    help_center_patterns = [
        "/help/", "/support/", "/customer/", "/gp/help/",
        "/customer-service/", "/customer-support/",
        "nodeId=", "helpId=", "topic=terms", "topic=legal" # Query parameters
    ]
    ...
    # Path fragments for user/customer terms
    user_customer_path_indicators = [
        "/user-terms", "/user_terms", "/user/terms",
        "/customer-terms", "/customer_terms", "/customer/terms",
        "/account/terms", "/profile/terms", "/user-agreement"
    ]
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if not has_terms_indicator:
            # logger.debug(f"Skipping link lacking terms indicator: '{text}' ({href})") # Optional: very verbose
            continue
        ...
        try:
            ...
            if any(term in text for term in user_customer_terms): score += 250; is_user_customer_related = True
            if "user terms of service" in text or "user terms of use" in text: score += 120
            if "customer terms of service" in text or "customer terms of use" in text: score += 100
            ...
            if any(term in query for term in ["user", "customer"]) and "terms" in query: score += 120; is_user_customer_related = True

            # == Standard Terms Scoring (added if not already high score from user/customer) ==
            ...
            if not is_user_customer_related or score < 150: # Adjusted threshold
                ...
                if text == "terms of service" or text == "terms of use": score += 100
                elif "terms of service" in text or "terms of use" in text: score += 80
                elif "terms and conditions" in text: score += 70
                elif "user agreement" in text or "legal terms" in text: score += 70
                elif "terms" in text: score += 50 # Contains "terms" gets decent score
                elif text == "legal": score += 30 # Exact match "legal" gets lower score

            # == URL Path/Query Scoring ==
            # Bonus for standard terms paths (can stack with user/customer score)
            ...
            # Help center terms pages (can stack) - Check path OR query
            ...
            if is_help_center_link:
                # Boost if terms terms are also present
                if "terms" in path or "terms" in query or "terms" in text:
                    score += 70
                    logger.debug(f"URL appears to be a help center terms page: {abs_url}")
                else:
                    score += 20 # Small boost just for being a help link, might contain terms info

            # Special case for parameter-based terms pages (can stack) - Check query parameters case-insensitively
            query_params_lower = {k.lower(): v for k, v in parse_qs(parsed_url.query).items()}
            if "nodeid" in query_params_lower or "helpid" in query_params_lower:
                score += 60
                logger.debug(f"URL appears to use nodeId/helpId parameter common in terms pages: {abs_url}")

            # == Contextual Scoring ==
            # Rel attributes (can stack)
            if "terms" in rels: score += 100
            if "legal" in rels: score += 80 # Often linked near terms
            ...
            # Penalize non-specific query parameters (but allow known terms/help ones)
            ...
            if has_unknown_query or has_content_id_param:
                ...
                if not is_help_center_link:
                    ...
                    logger.debug(f"Applying penalty for potentially non-terms query parameters: {query}")
            ...
            logger.warning(f"Error processing link href='{href}' on base '{base_url}': {e}", exc_info=False) # exc_info=False for less noise
            continue
    ...
    if candidate_links:
        logger.info(f"Found {len(candidate_links)} potential terms link candidates after scoring")
