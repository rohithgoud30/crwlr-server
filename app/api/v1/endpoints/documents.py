...
@router.post("/documents/sync-typesense", response_model=Dict[str, Any])
async def sync_documents_to_typesense(
    api_key: str = Depends(get_api_key)
):
    """
    Synchronize all documents from Firebase to Typesense.
    This is useful for initial setup or when rebuilding the search index.
    ...
    """
    try:
        logger.info("Starting document synchronization to Typesense")
        result = await document_crud.sync_all_documents_to_typesense()
        
        if result["success"]:
            logger.info(f"Document synchronization completed: {result['indexed']}/{result['total']} documents indexed")
        else:
            logger.error(f"Document synchronization failed: {result['message']}")
        ...
        logger.error(f"Error during document synchronization: {str(e)}")
        return {
            "success": False,
